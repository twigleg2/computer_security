...
bits = { # bit masks to be used in ffm
    0: 0x1,
    1: 0x2,
    2: 0x4,
    3: 0x8,
...
def getXY(byte):
    x = byte >> 4 #half a byte
    y = byte & 0x0f
    return x, y


def subWord(word):
    byte0 = word & 0x000000ff
    byte1 = (word >> 1*8) & 0x000000ff
    byte2 = (word >> 2*8) & 0x000000ff
    byte3 = (word >> 3*8) & 0x000000ff
    ...
    newWord = 0
    newWord = byte3
    newWord = (newWord << 8) + byte2
    newWord = (newWord << 8) + byte1
    newWord = (newWord << 8) + byte0
    return newWord


def rotWord(word):
    topByte = word >> 3*8
    newWord = (word << 8) & 0x00ffffffff
    newWord += topByte
    return newWord


def makeMatrix(array) #takes an array and turns it into a 4x4 matrix

def keyExpansion(key, expandedKey):
    pass
